...
*/
	...
	$query = "SELECT * FROM $daddb.$table WHERE id=$id";
	...
	echo '<form name="adminedit" action="edit_do.php?id='.$id.'&table='.$table.'" method="post"
	...
	while ($i < $fields) {
		...
		if (in_array($meta->name,$fknames)) {
			...
			$fkquery2 = "SELECT * FROM $daddb.$fktables[$fkindex]";
			$fkresult2 = mysql_query($fkquery2) or die(mysql_error());

			$original = mysql_fetch_array(mysql_query("SELECT name FROM $daddb.$fktables[$fkindex]
				WHERE id=$row[$i]"));

			if ($meta->name == "user") $original = mysql_fetch_array(mysql_query("SELECT email
				FROM $fktables[$fkindex] WHERE id=$row[$i]"));
